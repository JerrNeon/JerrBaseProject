...
import com.cw.andoridmvp.pulltorefresh.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
     ...
     */
    private static final String UPDATED_AT = "updated_pull";
    /**
     * 用于存储上次更新时间
     */
    ...
    public final boolean onTouchEvent(MotionEvent ev) {
        //读取上次更新时间并且显示
        setLastUpdatedLabel(formatDateTime(preferences.getLong(UPDATED_AT + saveTimeKey, 0)));
    ...
    @Override
    public void setOnRefreshListener(OnRefreshListener<T> refreshListener, int saveTimeKey) {
        mRefreshListener = refreshListener;
        this.saveTimeKey = saveTimeKey;
    }

    @Override
    public void onPullDownRefreshComplete() {
        onPullDownRefreshCompleteFailure(true);
    }

    @Override
    public void onPullDownRefreshCompleteFailure(boolean isSuccess) {
        if (isPullRefreshing()) {
            //保存用户上次更新时间
            preferences.edit()
                    .putLong(UPDATED_AT + saveTimeKey, System.currentTimeMillis())
                    .commit();
            mHeaderLayout.setRefreshingLabel("刷新成功");
            if(isSuccess) {  //成功接收刷新就停留1秒
                // 刷新结果停留1秒
                new Handler() {
                    @Override
